...
真实用户点击卡片按钮触发的 `card.action.trigger` 平台投递**无法在无人环境自动触发**——这是飞书平台限制（平台语义绑定真人点击，官方只提供“接收回调”能力，无“制造用户交互”的服务端 API；Codex + 官方文档查证 2026-07-22）。两个成品级参考仓（cc-connect / lcab）同样不真测此链路。其**处理逻辑**已由 L1 确定性测试 + L2 render 注入覆盖；需要真实中断时用文本 `/stop` 替代点按钮。
