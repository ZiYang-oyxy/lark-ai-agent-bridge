...
- Coder（兼容 key `append`）两轮创建两张卡并内联保留全部进展/工具；Worker（`append-clean-card`）最终正文只留末答、上下时间线保留进展/工具及完整计数；Singleton（`latest-card`）同样保留过程区，并在同 scope 复用 card ID、递增 sequence。
