...
- **回复语义**:`append` 每轮新建卡并保留本轮全部 assistant 回复与思考/工具过程区;`append-clean-card` 每轮新建卡,终态只保留最后一段 assistant 回复并移除过程区;`latest-card` 只额外按 conversation scope 复用卡片,其他终态语义与 `append-clean-card` 相同,跨重启继续递增 sequence。旧卡 ID 失效时清除 mapping 并新建卡,真实飞书返回的 `10002 cardid invalid` 已纳入 stale 判定。
