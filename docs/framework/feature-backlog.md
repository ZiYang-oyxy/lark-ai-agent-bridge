...
- **状态(2026-07-18)**:✅ 已完成。`/config` 可选择 Coder、Worker、Singleton；兼容 key 仍为 `append`、`append-clean-card`、`latest-card`，偏好与 Singleton mapping 使用原子 JSON 持久化。
- **回复语义**:Coder 每轮新建 CardKit,按原始顺序内联保留 assistant 进展、工具摘要和最终回复；Worker 每轮新建完整卡片,最终正文只保留末答,思考/进展与工具时间线保留最近两条并显示完整计数；Singleton 按 conversation scope 复用卡片,最终正文只保留末答并继续保留折叠过程区。跨重启继续递增 sequence；旧卡 ID 失效时清除 mapping 并新建卡,真实飞书返回的 `10002 cardid invalid` 已纳入 stale 判定。
