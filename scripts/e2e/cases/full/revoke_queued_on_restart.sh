#!/usr/bin/env bash
# full: revoke_queued_on_restart

case_revoke_queued_on_restart() {
  require_fake_claude
  ...
  file="$(mget revoke_queued_on_restart "$follow")"
  assert_file_contains "$file" "FAKE_E2E_STARTED"
  record_message revoke_queued_on_restart active "$active"
  record_message revoke_queued_on_restart cleared_queue "$queued"
  record_message revoke_queued_on_restart followup "$follow" "$file"
}
