...
//   - an optional one-line grey hint (text_size notation); omitted when empty
//   - the control (select/button/etc.); appended only when non-nil
func fieldElements(label, hint string, control map[string]any) []map[string]any {
	elements := []map[string]any{
		markdownElement("", fmt.Sprintf("**%s**", label)),
	}
	if hint != "" {
		elements = append(elements, noteElement("", hint))
	}
	if control != nil {
