...
func (s *Service) handleHelpCommand(ctx context.Context, msg Message, preference config.RuntimePreference) error {
	event := s.helpUpdateEvent(ctx, runID("help", msg.ID), msg.ID, preference.ConversationMode)
	return s.Cards.Render(event)
}

func (s *Service) helpUpdateEvent(ctx context.Context, sessionID, replyToMessageID string, mode config.ConversationMode) card.Event {
	...
	event := card.Event{
		Type: "update_help", SessionID: sessionID, ReplyToMessageID: replyToMessageID,
		ReplyInThread: mode == config.ConversationModeTopic,
		HeaderTitle:   "Bridge 帮助", HeaderTemplate: "blue",
	}
	if s.Updates == nil {
