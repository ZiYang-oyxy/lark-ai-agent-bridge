		...
		Groups: []card.HelpGroup{
			{
				...
				Lines: []string{
					...
					"**`/mkdir`** `[path]` 在 workspace 范围内创建目录（仅管理员）",
				},
			},
			{
				Title: "⚙️ 配置",
				Lines: []string{
					"**`/config`** 全局运行偏好 · **`/config set`** `key=value ...` 修改",
					"**`/local-config`** `[reset]` 本群覆盖 · **`/local-config set`** `key=value|inherit ...` 修改",
				},
			},
			{
				Title: "⏰ 定时",
...
func HelpText() string {
	return strings.Join([]string{
		...
		"/mkdir <path> - create a directory inside the configured workspace root (admin only)",
		...
		"/cron confirm|cancel [draft-id] - confirm or cancel a pending recurring task draft",
		...
		"/timer confirm|cancel [draft-id] - confirm or cancel a pending one-shot task draft",
		"/timer info|del <id> - inspect or delete a one-shot task",
		"/config - configure global defaults",
		"/config set key=value ... - modify global defaults",
		"/local-config [reset] - override or reset this group's inherited defaults",
		"/local-config set key=value|inherit ... - modify this group's overrides",
