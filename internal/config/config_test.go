...
func TestLoadFromEnvParsesRuntimeTuning(t *testing.T) {
	...
	t.Setenv("E2E_QUEUE_QUIET_POLLS", "3")
	t.Setenv("E2E_CARD_MAX_CHARS", "4096")
	t.Setenv("E2E_INTERACTION_TIMEOUT_SEC", "15")
	t.Setenv("E2E_IDLE_REMINDER_AFTER_SEC", "30")
	t.Setenv("E2E_IDLE_CHECK_MS", "500")
	...
	if cfg.QueueQuietPolls != 3 {
		t.Fatalf("quiet polls = %d, want 3", cfg.QueueQuietPolls)
	}
	...
	if cfg.IdleReminderAfter != 30*time.Second {
		t.Fatalf("idle reminder after = %s, want 30s", cfg.IdleReminderAfter)
	}
	if cfg.IdleCheckEvery != 500*time.Millisecond {
		t.Fatalf("idle check interval = %s, want 500ms", cfg.IdleCheckEvery)
	}
